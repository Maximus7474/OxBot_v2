...
export default {
  ...
  execute: async (client, auditLogEntry, guild) => {
    try {
      if (AuditLogEvent.MemberBanAdd !== auditLogEntry.action) return;

      if (!auditLogEntry.executorId || !auditLogEntry.targetId) {
        return logger.info('[Event Member Ban] Executor ID or target ID is missing from the audit log entry.');
      }
      ...
      if (!executor || !targetUser) {
        return logger.info('[Event Member Ban] Executor or target user is missing from the audit log entry.');
      }
      ...
      if (!channel || !channel.isSendable()) {
        logger.error(
          `[Event Member Ban] Action log channel (${config.channels.messagelog}) is missing or not sendable.`,
        );
        return;
      }
      ...
      if (channel) {
        try {
          ...
          logger.error('[Event Member Ban] Error sending embed:', error);
        }
      }
    } catch (err) {
      logger.error('[Event Member Ban] Unable to process ban', auditLogEntry.id, (err as Error).message);
    }
  },
